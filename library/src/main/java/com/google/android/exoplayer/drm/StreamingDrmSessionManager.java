...
public class StreamingDrmSessionManager implements DrmSessionManager {
  ...
  public final void open(DrmInitData drmInitData) {
    ...
    if (schemeData == null) {
      ...
      if (AmazonQuirks.isAmazonDevice() && uuid.equals(PLAYREADY_UUID)) {
        // PlayReady support on Amazon devices require the PlayReady header to
        // be extracted from the PSSH
        extractSchemeSpecificData = true;
      }

      if (extractSchemeSpecificData == true) {
        byte[] psshData = PsshAtomUtil.parseSchemeSpecificData(schemeData, uuid);
        if (psshData == null) {
