...
public abstract class MediaCodecTrackRenderer extends SampleSourceTrackRenderer {
  ...
  protected String codecName = "CodecNameUnknown"; // AMZN_CHANGE_ONELINE
    ...
    if (drmInitData != null) {
      ...
      if (drmSessionState == DrmSessionManager.STATE_ERROR) {
        ...
        if (AmazonQuirks.waitForDRMKeysBeforeInitCodec()) {
          canInitCodec = (drmSessionState == DrmSessionManager.STATE_OPENED_WITH_KEYS);
        } else {
          canInitCodec = (drmSessionState == DrmSessionManager.STATE_OPENED ||
  ...
  protected void doSomeWork(long positionUs, long elapsedRealtimeUs, boolean sourceIsReady)
    ...
    if (log.allowVerbose()) {
      log.v("doSomeWork: positionUs = " + positionUs
              + " elapsedRealtimeUs = " + elapsedRealtimeUs
              + " sourceIsReady = " + sourceIsReady);
    }
    if (format == null) {
  ...
  private boolean drainOutputBuffer(long positionUs, long elapsedRealtimeUs)
    ...
    if (outputIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
      ...
      if (log.allowVerbose()) {
        log.v("dequeueOutputBuffer: outputIndex = " + outputIndex);
      }
  ...
  private static boolean codecNeedsEosPropagationWorkaround(String name) {
    return AmazonQuirks.codecNeedsEosPropagationWorkaround(name) ||
             ( Util.SDK_INT <= 17 && "OMX.rk.video_decoder.avc".equals(name) ); // AMZN_CHANGE_ONELINE
  }
