...
public class MediaCodecAudioTrackRenderer extends MediaCodecTrackRenderer implements MediaClock {
  ...
  protected DecoderInfo getDecoderInfo(MediaCodecSelector mediaCodecSelector, String mimeType,
    ...
    if (allowPassthrough(mimeType) && AmazonQuirks.useDefaultPassthroughDecoder()) { // AMZN_CHANGE_ONELINE {
  ...
  protected void onOutputFormatChanged(android.media.MediaFormat outputFormat) {
    Log.i(TAG, "onOutputFormatChanged: inputFormat = outputFormat = " + outputFormat);
    // AMZN_CHANGE_BEGIN
    ...
    audioTrack.configure(passthrough ? passthroughMediaFormat : outputFormat,
            (passthrough || MimeTypes.isPassthroughAudio(outputMimeType)) );
    // AMZN_CHANGE_END
  }
  ...
  protected boolean isReady() {
    return audioTrack.hasPendingData() ||
            (super.isReady() &&
               (audioTrack.isInitialized() || getSourceState() == SOURCE_STATE_READY_READ_MAY_FAIL));
  }
