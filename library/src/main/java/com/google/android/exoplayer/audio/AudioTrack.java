...
public final class AudioTrack {
  ...
  public boolean isDolbyPassthroughQuirkEnabled() {
  ...
  public long getCurrentPositionUs(boolean sourceEnded) {
    ...
    if (audioTrack.getPlayState() == android.media.AudioTrack.PLAYSTATE_PLAYING
        && !isDolbyPassthroughQuirkEnabled() ) { // AMZN_CHANGE_ONELINE
  ...
  public void reconfigure(MediaFormat format, int specifiedBufferSize) {
    ...
    // We use this to override the encoding format
    // because in fire tv family we support DirectTrack for Dolby and the encoding
    // format has different values as compared to Android.
    if (getDirectTrackAudioFormat != null && isAc3) {
  ...
  public int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
    ...
    if (temporaryBufferSize == 0) {
      ...
      if (Util.SDK_INT < 21 || isDolbyPassthroughQuirkEnabled()) { // AMZN_CHANGE_ONELINE
  ...
  public boolean hasPendingData() {
    ...
    boolean isDataPending = isDolbyPassthroughQuirkEnabled() ||
  ...
  public boolean hasEnoughDataToBeginPlayback() {
    ...
    boolean ret = (isDolbyPassthroughQuirkEnabled() || (submittedBytes > (minBufferSize * 3) / 2));
