...
/* package */ final class AudioTrackPositionTracker {
  ...
  private static final String TAG = AudioTrackPositionTracker.class.getSimpleName(); // AMZN_ONELINE_CHANGE
  ...
  public AudioTrackPositionTracker(Listener listener,
    ...
    if (Util.SDK_INT >= 18) {
      try {
        ...
        Log.w(TAG, "Some Amazon legacy devices throw unexpected errors", e);
      }
    }
    ...
    if (applyDolbyPassThroughQuirk) {
      ...
      if (VDBG) {
        log.v("getCurrentPositionUs : hasTimestamp: positionUs = "  + positionUs +
            ...
            " timestampSysTimeUs  = " + audioTimestampPoller.getTimestampSystemTimeUs());
      }
      // AMZN_CHANGE_END
    } else {
